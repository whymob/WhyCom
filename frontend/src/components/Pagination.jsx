...
export default function Pagination({ page, pages, total, pageSize, onPageChange }) {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--wc-border)] px-4 py-3 text-xs text-slate-500">
      ...
      <div className="flex flex-wrap items-center gap-1">
        ...
        {firstPages.map((number) => (
          <Button key={number} variant={number === page ? "default" : "outline"} size="sm" onClick={() => onPageChange(number)} className="min-w-8 px-2">{number}</Button>
        ))}
        {pages > 4 && page > 5 && <span className="px-1">…</span>}
        {showCurrent && !firstPages.includes(page) && <Button variant="default" size="sm" className="min-w-8 px-2">{page}</Button>}
        {pages > 5 && page < pages - 1 && <span className="px-1">…</span>}
        {pages > 4 && <Button variant={page === pages ? "default" : "outline"} size="sm" onClick={() => onPageChange(pages)} className="min-w-8 px-2">{pages}</Button>}

...
export default function Invoices() {
  ...
  const exportInvoices = async () => {
    setExporting(true);
    try {
      const response = await api.get("/exports/invoices.xlsx", { params: { search: filters.search, collection_status: filters.statuses.join(","), sort_by: sort.key, sort_dir: sort.direction }, responseType: "blob" });
      ...
      toast.success("Faturas exportadas para Excel");
    } catch {
      toast.error("Não foi possível exportar as faturas.");
    } finally {
      setExporting(false);
    }
  };
  ...
  return <div>
    <PageHeader kicker="Financeiro" title="Faturas" actions={<><ListFilterSettings filters={filters} statusOptions={COLLECTION_OPTIONS} filterLabel="Situação de recebimento predefinida" onSave={saveFilters} onClear={clearSavedFilters} /><button type="button" onClick={exportInvoices} disabled={exporting} data-testid="export-invoices-xlsx" className="inline-flex h-10 items-center rounded-none border border-neutral-300 bg-white px-3 text-sm font-medium hover:bg-neutral-50 disabled:opacity-60"><Download size={16} className="mr-1" />{exporting ? "A exportar…" : "Exportar Excel"}</button></>} />
    <div className="p-7"><div className="wc-list-panel overflow-hidden">

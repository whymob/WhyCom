  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<>
          {isManager && <select value={ownerId} onChange={(event) => { setOwnerId(event.target.value); load(event.target.value); }} className="h-9 border border-neutral-300 bg-white px-2 text-xs"><option value="">Toda a equipa</option>{owners.map((owner) => <option key={owner.id} value={owner.id}>{owner.name}</option>)}</select>}
          <button onClick={() => load()} className="flex items-center gap-2 border border-[#002FA7] px-3 py-2 text-xs text-[#002FA7] hover:bg-[#002FA7] hover:text-white"><RefreshCw size={14} /> Atualizar</button>
        </>}
      />
      <div className="p-8">
        <div className="mb-5 text-sm text-neutral-600">Os processos ficam lado a lado. Pode expandir vários grupos; quando necessário, use a barra horizontal para navegar. Estados de encerramento só podem ser definidos pelas ações do registo, nunca por arrasto.</div>
        ...
        <div className="overflow-x-auto pb-4">
          <div className="flex min-w-max items-start gap-4">
            {board?.columns?.map((process) => {
              ...
              return <section key={process.key} onDragOver={(event) => { if (isDropTarget) event.preventDefault(); }} onDrop={(event) => { event.preventDefault(); requestAdvance(process.key); }} className={`shrink-0 border bg-neutral-50 ${expanded ? "w-auto" : "w-72"} ${isDropTarget ? "border-[#002FA7] ring-1 ring-[#002FA7]" : "border-neutral-200"}`}>
                <div className="flex items-center justify-between gap-5 border-b border-neutral-200 bg-white px-4 py-3"><div><div className="text-[10px] uppercase tracking-[0.18em] text-neutral-500">Processo</div><div className="mt-1 text-sm font-medium">{process.label}</div><div className="mt-2 font-mono text-xs text-neutral-700">{process.count} registo(s) · {eur(process.value)}</div></div><button type="button" onClick={() => toggleProcess(process.key)} className="flex items-center gap-1 border border-neutral-300 px-2.5 py-2 text-xs text-neutral-700 hover:bg-neutral-50">{expanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}{expanded ? "Recolher" : "Expandir"}</button></div>
                {(expanded || ["leads", "opportunities", "proposals", "orders"].includes(process.key)) && <div className="p-3"><div className="flex min-w-max gap-3">
                  {visibleGroups.map((group) => <div key={group.status} onDragOver={(event) => { if (dragged && canMoveWithinProcess(dragged.item, dragged.processKey, group.status)) event.preventDefault(); }} onDrop={(event) => { if (dragged && canMoveWithinProcess(dragged.item, dragged.processKey, group.status)) { event.preventDefault(); event.stopPropagation(); requestStatusMove(group.status); } }} className={`w-64 shrink-0 border bg-white ${dragged && canMoveWithinProcess(dragged.item, dragged.processKey, group.status) ? "border-[#002FA7] ring-1 ring-[#002FA7]" : "border-neutral-200"}`}>
                    <div className="flex items-center justify-between border-b border-neutral-200 bg-neutral-50 px-3 py-2"><div className="text-[10px] uppercase tracking-widest text-neutral-500">{statusLabel({ kind: kindForProcess(process.key), status: group.status })}</div><div className="flex items-center gap-1 font-mono text-xs text-[#002FA7]"><span>{group.items.length}</span>{FINAL_STATUS[process.key] === group.status && <ArrowRight size={12} title="Pronto para avançar" />}</div></div>
                    <div className="min-h-24 space-y-3 p-3">{group.items.length === 0 && <div className="py-4 text-center text-xs text-neutral-400">Sem registos.</div>}{group.items.map((item) => {
                      ...
                      return <button key={item.id} type="button" draggable={itemCanDrag} onDragStart={() => itemCanDrag && setDragged({ item, processKey: process.key })} onDragEnd={() => setDragged(null)} onClick={() => setSelected(item)} className={`block w-full border bg-white p-3 text-left hover:border-[#002FA7] ${dragged?.item.id === item.id ? "opacity-40" : "border-neutral-200"} ${itemCanDrag ? "cursor-grab" : "cursor-pointer"}`} title={dragTitle}>
